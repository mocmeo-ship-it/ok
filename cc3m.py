    ...
    def send_request_using_proxy(self, url, headers=None, ciphers=None, tls_params=None, timeout=None):
        ...
        try:
            ...
            if tls_params:
                ...
                session.verify = tls_params.get('verify', True)
                session.cert = tls_params.get('cert')
                session.ssl_version = getattr(ssl, tls_params.get('ssl_version'))
                session.verify = tls_params.get('verify', True)
                session.cert = tls_params.get('cert')
                session.ssl_version = tls_params.get('ssl_version')
                session.verify_mode = ssl.CERT_OPTIONAL if tls_params.get('verify', True) else ssl.CERT_NONE
                if 'cafile' in tls_params:
                    session.verify = tls_params['cafile']
...
import sys
tls_params = {
    'verify': True,            # Kiểm tra chứng chỉ
    
    'ssl_version': 'PROTOCOL_TLSv1_2',      # Phiên bản TLS

}
timeout = 5
at = input("file prx: ") #'a.txt'
attacker = Attacker(file_path=at)
